...
describe('Task: sequential - ember addon', () => {
  for (const [name, originalProject] of Object.entries(projects)) {
    describe(name, () => {
      ...
      test('sequential run regen on the existing report, and run migrate on current base path and entrypoint', async () => {
        ...
        const addonComponentsFiles = readdirSync(addonComponentsPath);
        expect(addonComponentsFiles).toContain('greet.hbs');
        expect(addonComponentsFiles).toContain('greet.ts');

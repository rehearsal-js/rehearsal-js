...
import { createDependencyGraph } from '../src/file-dependency-graph/basic';
...
function flatten(arr: GraphNode<ModuleNode>[]): string[] {
...
describe('Basic: createDependencyGraph', () => {
  ...
  test('provide graph for a complex library', async () => {
    const baseUrl = getLibrarySimple();
    const output: Graph<ModuleNode> = createDependencyGraph(baseUrl, './index.js');
    const paths: string[] = [];

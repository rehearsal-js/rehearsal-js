...
describe('Suppressed Errors', function () {
  const cases = [
    [
      `TS2345: Cannot read properties of undefined (reading 'flags')`,
      'suppressed-errors/flags.ts',
    ...
    [
      ...
      'suppressed-errors/missingMethod.ts',
      `// @ts-expect-error @rehearsal TODO TS2339: Property 'setRoutes' does not exist on type 'Something'.`,
    ],
  ];

  test.each(cases)('%s, %s', async (...args) => {
    const [, relativePathToFixture, expected] = args;

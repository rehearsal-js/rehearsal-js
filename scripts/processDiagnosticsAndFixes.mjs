...
function buildDiagnosticFileOutput(messageTable, inputFilePathRel) {
  const result = [
    ...
    '',
    'import { type DiagnosticCategory as DiagnosticCategoryType } from "typescript"; ',
    '// this is a workaround until ts 5.0.0',
    ...
    '}',
    '',
    'function diag(code: number, category: DiagnosticCategoryType, key: string, message: string, reportsUnnecessary?: {}, elidedInCompatabilityPyramid?: boolean, reportsDeprecated?: {}): DiagnosticMessage {',
  ...
  messageTable.forEach(
    (
      { code, category, reportsUnnecessary, elidedInCompatabilityPyramid, reportsDeprecated },
      name
    ) => {
...
function createDiagnostics(diagnostics, diagnosticJson) {
  ...
  return diagnostics.map((code) => `Diagnostics.${diagnosticsByCode[code]}`).join(',');
}
